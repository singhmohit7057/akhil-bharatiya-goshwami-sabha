...
export function EditEvent() {
    ...
    supabase.from('events').select('*').eq('id', id).single().then(({ data }) => {
      if (data) {
        ...
        setForm({
          ...
          end_time: e.end_date ? (e.end_date.slice(11, 16) !== '00:00' ? e.end_date.slice(11, 16) : '') : '',
          location: e.location || '',
        })
        if (e.image_url) setBannerPreview(e.image_url)
      }
      setLoading(false)
    })
  }, [id])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (bannerFile) {
      ...
      const { error: uploadError } = await supabase.storage.from('event-images').upload(path, bannerFile)
      if (!uploadError) {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-text-primary mb-1">Edit Event</h1>
      <p className="text-sm text-text-secondary mb-6">Update event details</p>
